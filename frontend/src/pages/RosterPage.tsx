    ...
    <div className="max-w-2xl">
      ...
      <div className="grid gap-3 mb-8">
        {roster?.slots.map(slot => (
          <div key={slot.slot_number} className="card flex items-center justify-between">
            <div>
              <span className="font-semibold">{slot.castaway.name}</span>
              {slot.castaway.is_eliminated && (
                <span className="ml-2 text-xs text-red-500">Eliminated Ep {slot.castaway.eliminated_episode}</span>
              )}

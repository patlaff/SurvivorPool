...
import { useMemberRoster, useLeague } from '../api/leagues'
...
export default function RosterViewPage() {
  ...
  const crumbs = [
    ...
    { label: roster ? `${roster.user.display_name}'s Roster` : '…' },
  ]

  if (isLoading) return (
    <div className="max-w-2xl">
      <Breadcrumbs crumbs={crumbs} />
      <div className="text-gray-400 dark:text-gray-500 py-8 text-center">Loading…</div>
    </div>
  )
  ...
  if (isError) {
      ...
      return (
        <div className="max-w-2xl">
          ...
          <div className="card text-center py-12 text-gray-500 dark:text-gray-400">
            ...
            <p className="text-sm mt-1 text-gray-400 dark:text-gray-500">Check back once the draft closes.</p>
          </div>
        </div>
      )
    }
    return (
      <div className="max-w-2xl">
        <Breadcrumbs crumbs={crumbs} />
        <div className="text-gray-400 dark:text-gray-500 py-8 text-center">Roster not found.</div>
      </div>
    )
  }
    ...
    <div className="max-w-2xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">{roster.total_points} pts total</p>
        </div>
      </div>
        ...
        {roster.slots.map(slot => (
          <div key={slot.slot_number} className="card flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {slot.castaway.occupation && <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{slot.castaway.occupation}</p>}
